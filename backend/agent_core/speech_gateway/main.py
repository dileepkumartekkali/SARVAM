...
async def converse_ws(
    ...
    async def run_turn(transcript: str) -> None:
        ...
        #
        # A second real bug hit live, confirmed via Sarvam TTS config logs:
        # this used to pass `transcript` (the user's OWN spoken question)
        # to resolve_tts_voice_language, not `reply.text` (the actual answer
        # about to be synthesized) -- resolve_tts_voice_language's own
        # romanized-input safety net (see its docstring) exists ONLY because
        # the FRONTEND's separate TTS path opens its socket before any reply
        # text exists, so it has nothing else to check. Here, `reply.text`
        # is already fully generated by this point -- confirmed live: for
        # "mtouch labs motive enti??" (a romanized-Telugu question), the
        # LLM answered in full NATIVE Telugu script (voice mode always
        # demands native script -- see task_agent._expected_script), but
        # checking the QUESTION's script (all Latin, since it was typed
        # romanized) picked the English voice anyway -- Sarvam TTS config
        # logged `target_language_code: 'en-IN'` while being sent Telugu
        # Unicode text chunks. Checking the actual text being spoken is
        # strictly more accurate whenever it's already known.
        tts_language = resolve_tts_voice_language(reply.text, reply.response_language)
