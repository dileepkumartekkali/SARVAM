...
| User-facing chat history | Supabase Postgres (`agent_core/persistence/chat_store.py`), written by a `BackgroundTasks` call (`record_turn`) **after** the `/chat` response is already sent — the user is never blocked on this write. Both messages in a turn share one DB transaction (never half-saved) and a failed attempt retries up to 3 times. **Accepted remaining risk**: if the backend process dies in the narrow window between sending the response and the background task completing (e.g. a Render redeploy), that turn's history is lost even though the user got a real answer — there's no durable job queue/outbox behind this, by deliberate choice (disproportionate complexity for non-critical chat history; see the commit message for `record_turn`). | Nothing across a process death mid-task (rare — the window is typically milliseconds). Survives everything else (network blips, transient Supabase errors) via the retry. |
